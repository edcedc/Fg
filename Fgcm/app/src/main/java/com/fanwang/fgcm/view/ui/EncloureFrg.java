...
import com.fanwang.fgcm.presenter.EnclourePresenterlmpl;
...
public class EncloureFrg extends BaseFragment<SNotTitleRecyclerBinding> implements IBaseListView {

    private ChangyongdeListPresenter presenter;

    public static EncloureFrg newInstance() {
        Bundle args = new Bundle();
        EncloureFrg fragment = new EncloureFrg();
    ...
    protected void initView(View view) {
        presenter = new EnclourePresenterlmpl(this);
        if (adapter == null) {
            adapter = new EncloureHotAdapter(act, this, listBean, BundleDataInEvent.mEncloureFabulous);
        }
        ...
        setRefreshLayout(mB.refreshLayout, new RefreshListenerAdapter() {
            ...
            public void onRefresh(TwinklingRefreshLayout refreshLayout) {
                ...
                presenter.onRequest(pagerNumber = 1);
            }
    ...
    public void onPraiseState(BundleDataInEvent event){
        if (event.getType() == BundleDataInEvent.mEncloureFabulous){
    ...
    public void onPlayback(BundleDataInEvent event){
        if (event.getType() == BundleDataInEvent.mEnclourePlay){
            int po = (int) event.getObject();
            if (po != -1){
                DataBean bean = listBean.get(po);
    ...
    public void onReb(BundleDataInEvent event){
        if (event.getType() == BundleDataInEvent.Encloure_Red){
            int po = (int) event.getObject();
            if (po != -1){
                DataBean bean = listBean.get(po);
