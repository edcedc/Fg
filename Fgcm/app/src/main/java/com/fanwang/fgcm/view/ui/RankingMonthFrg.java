...
public class RankingMonthFrg extends BaseFragment<SNotTitleRecyclerBinding>implements IBaseListView {

    private ChangyongdeListPresenter presenter;

    public static RankingMonthFrg newInstance() {
        Bundle args = new Bundle();
        RankingMonthFrg fragment = new RankingMonthFrg();
    ...
    private final int type = BundleDataInEvent.monthRanking;
    ...
    protected void initView(View view) {
        ...
        setRefreshLayout(mB.refreshLayout, new RefreshListenerAdapter() {
            ...
            public void onRefresh(TwinklingRefreshLayout refreshLayout) {
                ...
                presenter.onRequest(pagerNumber = 1, CloudApi.videoPageMonth);
            }
    ...
    public void onPraiseState(BundleDataInEvent event){
        if (event.getType() == BundleDataInEvent.monthRanking){
    ...
    public void onRebGone(BundleDataInEvent event){
        if (event.getType() == BundleDataInEvent.monthRankingRed){
            int po = (int) event.getObject();
            if (po != -1){
                DataBean bean = listBean.get(po);
    ...
    public void onReb(BundleDataInEvent event){
        if (event.getType() == BundleDataInEvent.monthRankingIsRed){
            int po = (int) event.getObject();
            if (po != -1){
                DataBean bean = listBean.get(po);
